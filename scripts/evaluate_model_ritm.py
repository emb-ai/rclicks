...
def get_checkpoints_list_and_logs_path(args, cfg):
    ...
    if args.user_inputs and args.clickability_model_pth is None:
        suffix = 'User'
    elif args.user_inputs and args.clickability_model_pth:
        suffix = 'SamplingUsersCount'
    elif args.clickability_model_pth:
        suffix = 'Trajectory_BOUNDS_' + '%.3f' % float(args.trajectory_sampling_prob_low) + '_%.3f' % float(args.trajectory_sampling_prob_high)
    else:
        suffix = 'Base'
    print("Mode: ", suffix)
    return checkpoints_list, Path(str(logs_path) + "_" + suffix), logs_prefix
...
def save_results(args, row_name, dataset_name, logs_path, logs_prefix, dataset_results,
    ...
    if args.print_ious:
        ...
        mean_ious = np.array([x[:min_num_clicks, :2] for x in all_ious]).mean(axis=0)[:, 0]
...
def get_prediction_vis_callback(logs_path, dataset_name, prob_thresh):
    ...
    def callback(image, gt_mask, pred_probs, sample_id, click_indx, clicks_list, clickmap=None):
        sample_path = save_path / f'{sample_id}_{click_indx}.jpg'
        prob_map = draw_probmap(pred_probs)
        
        clickmap = (clickmap - clickmap.min()) / (clickmap.max() - clickmap.min() + 1e-6)
        clickmap = draw_probmap(clickmap)
  
        image_with_mask = draw_with_blend_and_clicks(image, pred_probs > prob_thresh, clicks_list=clicks_list)
        cv2.imwrite(str(sample_path), np.concatenate((np.clip(image_with_mask * 0.5 + clickmap * 0.5, 0 , 255), prob_map, clickmap), axis=1)[:, :, ::-1])
